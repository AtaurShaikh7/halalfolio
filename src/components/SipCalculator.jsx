...
export function SipCalculator({ r }) {
  ...
  return (
    <div className="rounded-2xl border bg-card shadow-card" style={{ borderColor: 'var(--border)' }}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-playfair text-[17px] font-semibold leading-tight">SIP Projection</span>
          ...
          {live && (
            <span
              ...
              style={{ color: 'var(--green)', background: 'rgba(46,204,113,0.12)', border: '1px solid rgba(46,204,113,0.32)' }}
            >
              <span className="h-1 w-1 rounded-full" style={{ background: 'var(--green)', boxShadow: '0 0 5px var(--green)' }} />
              LIVE
            </span>
          )}
        </div>
        <div className="flex items-center gap-3">
          {!open && (
            <span className="text-[12px] text-text2 tabular-nums hidden sm:block">
              Sharia{' '}
              <span className="font-semibold" style={{ color: 'var(--green)' }}>
                {fmtINRCompact(results.sharia.value)}
              </span>
              {' · '}Original{' '}
              <span className="font-semibold" style={{ color: 'var(--blue)' }}>
                {fmtINRCompact(results.original.value)}
              </span>
              {' '}at ₹{(+amount).toLocaleString('en-IN')}/mo
            </span>
          )}
          {open
            ? <ChevronUp size={16} className="text-text2 flex-shrink-0" />
            : <ChevronDown size={16} className="text-text2 flex-shrink-0" />}
        </div>
      </button>
        ...
        <div className="px-5 pb-5">
          <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
            ...
            <div className="flex items-center gap-2">
              <label className="text-[11px] uppercase tracking-wider text-text2">Monthly SIP</label>
              <div
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setAmount(p)}
                  className={`h-7 rounded-full px-3 text-[11.5px] font-semibold transition-colors ${active ? 'text-[#1a1610]' : 'text-text2 hover:text-text'}`}
                  style={active
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="text-[11px] uppercase tracking-wider text-text2 font-semibold">Inflation</span>
            {INFLATION_PRESETS.map((p) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setInfl(p)}
                  className={`h-7 rounded-full px-3 text-[11.5px] font-semibold tabular-nums transition-colors ${active ? 'text-[#1a1610]' : 'text-text2 hover:text-text'}`}
                  style={active
                    ? { background: 'linear-gradient(135deg, var(--gold2), var(--gold))' }
                    : { background: 'var(--card2)', border: '1px solid var(--border)' }}
                >
            ...
            <span className="text-[11.5px] text-text2">discounts future value to today's purchasing power</span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {cards.map((c) => {
              ...
              return (
                ...
                >
                  <span className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: c.color }} />
                  <div className="flex items-baseline justify-between">
                    ...
                    <div className="text-[11px] tabular-nums" style={{ color: c.color }}>×{mult.toFixed(2)}</div>
                  </div>
                  <div
                  ...
                  <div className="mt-1.5 text-[11.5px] text-text2 tabular-nums">
                    Gain{' '}
                    <span className="font-semibold" style={{ color: c.color }}>{fmtINR(gain)}</span>
                  </div>
                  <div className="mt-2.5 pt-2.5 border-t" style={{ borderColor: 'var(--border2)' }}>
                    <div className="text-[10px] uppercase tracking-wider text-text3">Worth in today's money</div>
                    <div className="mt-0.5 flex items-baseline gap-1.5">
                      <span className="font-playfair font-semibold text-[16px] tabular-nums text-text">
                        {fmtINRCompact(real)}
                      </span>
                      <span className="text-[10.5px] text-down tabular-nums">−{erosion.toFixed(0)}%</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-[12px] text-text2">
            <div>
              You'd invest{' '}
              <span className="font-semibold text-text tabular-nums">{fmtINR(invested)}</span>
              {' '}over{' '}
